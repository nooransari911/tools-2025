...
# Use $HOME for robust path expansion
CLOUDFRONT_INVALIDATE_SCRIPT="$HOME/Downloads/tools-2025/projects/CMS/invalidate_test.sh"
...
if [ -f "$AI_WORKING_DIR/.env" ]; then
    # This safely loads variables like AWS_API_GW_API_KEY
    set -a; source "$AI_WORKING_DIR/.env"; set +a
fi
...
echo "--- Solution Starting: Two-Script Orchestration (Final Fixes) ---"
...
inotifywait -m -e create --format '%w%f' "$SOURCE_DIR_A" "$OUTPUT_DIR" | while read -r NEW_FILE; do
    ...
    # --- TRIGGER 2: New OCR markdown file arrives -> Call Fireworks Script ---
        ...
        while (( ${#OCR_FILE_QUEUE[@]} > 0 )); do
            ...
            if aws s3 cp "$FINAL_OUTPUT_FILE" "$S3_TARGET"; then
                ...
                # --- Robust call to the invalidation script ---
                echo "--> Invalidating CloudFront cache..."

                # Ensure the key is loaded before trying to pass it
                if [ -z "$AWS_API_GW_API_KEY" ]; then
                    echo ">>> ERROR: AWS_API_GW_API_KEY is not set. Cannot invalidate."
                    continue
                fi

                chmod +x "$CLOUDFRONT_INVALIDATE_SCRIPT"

                # Pass the key as the first argument to the script
                if bash "$CLOUDFRONT_INVALIDATE_SCRIPT" "$AWS_API_GW_API_KEY"; then
                    echo ">>> CloudFront invalidation request sent successfully."
                else
                    echo ">>> ERROR: CloudFront invalidation script failed."
                fi
    ...
    # --- CORRECTED PLACEMENT: Explicitly ignore the final analysis file to prevent loops ---
    elif [[ "$NEW_FILE" == *final-analysis-*.md ]]; then
        echo "[Monitor] Final analysis file created. IGNORING to prevent infinite loop."
        echo "---------------------------"

    # --- Optional: A fallback for any other unexpected files in the output directory ---
    elif [[ "$NEW_FILE" == "$OUTPUT_DIR"* ]]; then
        echo "[Monitor] Unrecognized file pattern in output dir. IGNORING: $(basename "$NEW_FILE")"
        echo "---------------------------"
    fi
done
