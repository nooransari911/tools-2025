...
export class CommonService {
    ...
    req_headers_string (req: Request): string {
        let req_headers_string: string = "<h1>Request Headers:</h1><p><ul>";
        let header: IncomingHttpHeaders;
        let var_header_string: string | undefined;
        ...
        for (const header in req.headers) {
            ...
            if (Array.isArray (headerval)) {
                // var_header_string = headerval.map (element => JSON.stringify (element)).join (', ');
                var_header_string = headerval.join (', ');
            }

            else {
                // var_header_string = JSON.stringify (headerval);
    ...
    res_headers_string (res: Response): string {
        let res_headers_string: string = "<h1>Response Headers:</h1><p><ul>";
        let res_headers: OutgoingHttpHeaders = res.getHeaders ();
        let var_header_string: string | number | any | undefined;
        // let header_string: string;
