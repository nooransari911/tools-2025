...
find "$base_dir1" "$base_dir2" -type f -name "*.ts" -exec bash -c 'cp "$1" "./md/$(basename "$1" .ts).md"' _ {} \;

# Define the output file
output_file="$base_dir/comb-file-select.md"

# Clear the output file before writing
> "$output_file"

# Loop through .ts files from both directories and append their contents to the output file
find "$base_dir1" "$base_dir2" -type f -name "*.ts" | while IFS= read -r file; do
