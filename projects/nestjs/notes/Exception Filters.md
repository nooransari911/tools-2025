...
The `response` argument defines the JSON response body. It can be a `string` or an `object` as described below. The `status` argument defines the [HTTP status code(https://developer.mozilla.org/en-US/docs/Web/HTTP/Status)].
...
The second method is to use the `APP_FILTER` token [as shown here(exception-filters#binding-filters)].
